...
if (isset($_GET['id'])) {
    ...
    $cid = $id1;
    $hits = new hits();
    $hits->catagory_count($cid);
} else {
    $id1 = $_POST['id']; // catagory id
    $cid = $id1;
    $hits = new hits();
    $hits->catagory_count($cid);
}
...
<body style="margin:0px; padding:0px;">
    ...
    <div class="row-fluid" style="margin:0 auto; width:70%;   min-height: 533px;">
        <div class="widget-body" style="padding: 10px 0;">
            <form action="vacancies.php" method="post" id="myForm">
                <?php
                $SAtable = "jobs_category";
                $ST = new SelectTableAll();
                $ST->SelectTbAll($SAtable);
                ?>
                <select style="width: 30%; color: rgb(71, 65, 65);" id="select2_12" name="id" onchange="submit()" tabindex="-1">
                    <option value="0" label="Change the Category"> </option>
                    <?php while ($select = mysql_fetch_array($SAquery)) { ?>
            ...
            <table class="dynamicTable table table-striped table-bordered table-primary table-condensed">
                <thead>
                    <tr>
                        ...
                        <th class="center">Closing Date</th>
                        <th class="center">Full View</th>
                    </tr>
                </thead>
                <tbody>
                    <?php while ($js = mysql_fetch_array($query3)) { ?>
                        <tr class="gradeX">
                            <td> #<?php echo $js['reference_number']; ?></td>
                            <td> 
                                <?php echo $js['job_title']; ?> 
                                <br/> 
                                <span class="comname">
                                    <?php
                                    $select_table2 = "users";
                                    $suser_id2 = $js['user_id'];
                                    $swhere2 = "id";

                                    $ST2 = new SelectTable();
                                    $ST2->SelectTb2($suser_id2, $select_table2, $swhere2);
                                    $company = mysql_fetch_array($query2);
                                    echo $company['company_name'];
                                    ?> </td> </span>
                            <td> <div class="tdfield"> <?php echo $js['job_discription']; ?> </div> </td>
                            <td class="center"><?php echo $js['closing_date']; ?></td>
                            <td class="center"> 
                                <a class="fancybox fancybox.iframe" href="views/jobview.php?id=<?php echo $js['id']; ?> "> View </a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
